...
class Tune1d:
    def __init__(
        self, variable, python_command, old_x, old_y, num_gp_steps, bounds, seed_pt=None
    ):
        if not DEPENDENCIES_INSTALLED:
            raise ImportError(
                "Required dependencies (torch, botorch, gpytorch) are not installed. "
    ...
    def optimize(self):
        ...
        dtype = torch.float32
        train_x = (
            torch.rand(1, 1, dtype=dtype, device=self.device)
            * (self.bounds[1, 0] - self.bounds[0, 0])
            + self.bounds[0, 0]
        ).to(self.device)
        train_y = torch.tensor([[self._acquire_data(train_x[0])]], dtype=dtype).to(
            self.device
        )

        for _ in range(self.num_gp_steps):
            model = self._initialize_model(train_x, train_y)
            new_x = self._optimize_acquisition_function(model, train_y, self.bounds).to(
                self.device
            )
            new_y = torch.tensor([[self._acquire_data(new_x[0])]], dtype=dtype).to(
                self.device
            )
            time.sleep(1)
...
class Tune2d:
    ...
    def _acquire_data(self, x):
        result = self.python_command(
            x.cpu().numpy()
        )  # Convert tensor to numpy array before passing to the function
        return result
    ...
    def optimize(self):
        ...
        dtype = torch.float32
        train_x = (
            torch.rand(1, 2, dtype=dtype, device=self.device)
...
class Tune3d:
    ...
    def optimize(self):
        ...
        dtype = torch.float32
        train_x = (
            torch.rand(1, 3, dtype=dtype, device=self.device)
            * (self.bounds[1] - self.bounds[0])
            + self.bounds[0]
        ).to(self.device)
        train_y = torch.tensor([[self._acquire_data(train_x[0])]], dtype=dtype).to(
